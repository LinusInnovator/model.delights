  ...
  blocks: [
    ...
    },
    {
      ...
      content: {
        ...
        professional: "When architecting `model.delights.pro` over our 11-day sprint, I realized that standing up a traditional database like Supabase or Postgres to track AI models was a trap. It would force me to become a full-time data janitor. So, we abandoned the database entirely.",
        academic: "Throughout the 11-day rapid algorithmic prototyping phase, it became evident that deploying a standard persistent storage layer (e.g., PostgreSQL) would introduce unacceptable administrative overhead. Consequently, persistent local state was bypassed."
      }
    },
    {
      id: "p4",
      ...
      content: {
        ...
        professional: "Instead, we implemented a stateless API hydration model. `fetchModels()` is wired directly to the OpenRouter API. Every 5 minutes, using Next.js Incremental Static Regeneration (ISR), the server wakes up, pulls the live telemetry of hundreds of models, parses their exact token pricing and context windows, and instantly rebuilds the entire frontend UI.",
        academic: "The architecture utilizes Edge-routed API hydration. By leveraging Next.js Incremental Static Regeneration (ISR) with a 300-second TTL, the application asynchronously polls OpenRouter endpoints, ingesting raw JSON telemetry and statically regenerating the DOM tree at the CDN level."
      },
